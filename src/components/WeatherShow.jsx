...
const WeatherShow = () => {
    ...
    const fetchGeoData = async () => {
        ...
        const response = await fetch(
            `https://api.openweathermap.org/data/2.5/weather?q=${cityName},IT&limit=2&appid=388a663d9cdcf908295112ed55cbaaa8`
        );
    ...
    const fetchWeatherData = async (data) => {
        try {
        const response = await fetch(
            `https://api.openweathermap.org/data/2.5/weather?lat=${data.lat}&lon=${data.lon}&lang=it&units=metric&appid=388a663d9cdcf908295112ed55cbaaa8`
        );
        if (response.status === 404) throw new Error("Risorsa non trovata");
        if (response.status >= 400 && response.status < 500) throw new Error("Errore lato Client");
        if (response.status >= 500 && response.status < 600) throw new Error("Errore lato Server");
